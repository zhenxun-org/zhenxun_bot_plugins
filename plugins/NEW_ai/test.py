            ...
            if msg.type == web.WSMsgType.TEXT:
                ...
                if messages['post_type'] in ['message']:
                    ...
                    if messages['message_type'] == "group":
                        ...
                        for msgs in messages['message']:
                            ...
                            if msgs['type'] == 'reply':
                                ...
                                async with ClientSession() as session:
                                    async with session.post(url="http://127.0.0.1:3000/get_msg", json=payload, headers={ 'Content-Type': 'application/json'}) as resp:
                                        result = await resp.json()
                                        messages['message'].append(result['data']['message'][0])
                                        print(f"引用内容: {result}typ: {type(result['data']['message'][0])}")
                                        continue
                        ...
                        print(f"收到来自{messages['group_id']}的{messages['sender']['nickname']}消息:{message}{get_url}")
                        #await _(group_id=messages['group_id'], user_id=messages['user_id'],uname=messages['sender']['nickname'],message=msg)
                        if message[:4] == 'echo':
                            await send_message(messages['group_id'], f"{message[4:]}", "/send_group_msg")
