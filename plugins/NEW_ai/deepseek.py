    ...
    async def send_message(
        ...
        if reply_text:
            self.chat_history.append(
                {"role": "assistant", "content": f"用户引用了一条消息:\n{reply_text}"}
            )  # 仅支持文字图片暂不支持
        if image_url:
            ...
            self.chat_history.append(
                {
                    "role": "assistant",
                    "content": f"用户发来了一张图片其内容为{iamge_read}",
                }
            )
        ...
        if tool_use:
            ...
            if message.split(":")[1].split("[")[0] == "/deepseek":
                ...
                self.api_key = os.getenv(
                    "HUANZOU_API_KEY"
                )  # 从幻宙官网获取效果不佳但有不稳定免费模型可白嫖https://phapi.furina.junmatec.cn
                ...
                return f"已切换模型至{self.model_name}"
            elif message.split(":")[1].split("[")[0] in [
                "清除对话记录",
                "-clear",
                "遗忘",
            ]:
                        ...
                        if code_block_type in ["json", "jso"]:
                            ...
                            if code_block_json["type"] == "python":
                                ...
                                await self.search_from_web(
                                    search_information=code_block_json["source"],
                                    user_id=user_id,
                                    answer=answer,
                                    question=question,
                                )
                            elif code_block_json["type"] == "draw":
                                await self.draw_picture(
                                    code=code_block_json["source"], question=question
                                )
                                return code_block_json["source"]
                            elif code_block_json["type"] == "gold":
                                await self.gold(
                                    gold=float(code_block_json["source"]),
                                    user_id=user_id,
                                    answer=answer,
                                    question=question,
                                )
                            ...
                            answer += f"```\n{true_content}\n```"
                            await MessageUtils.build_message(
                                f"base64://{base64.b64encode(await md_to_pic(f'```{true_content}```', screenshot_timeout=9999999)).decode('utf-8')}"
                            ).send()
    ...
    async def see_picture(self, image_url):
        ...
        with open(save_path, "rb") as file:
            ...
            message = [
                {
                    "role": "system",
                    "content": "你需要描述你所看到的图像描述要简短清晰如果是文字类图片直接返回所有文字，注意不要加你的见解！！！！",
                },
                {
                    "role": "user",
                    "content": [
            ...
            response = await client.chat.completions.create(
                model="Qwen/Qwen2.5-VL-32B-Instruct", messages=message
            )
    ...
    async def run_python(self, code, user_id, question, answer):
        ...
        if "代码执行次数已达上限" not in code_output:
            ...
            await self.send_message(
                message=messages, user_id=user_id, question=question
            )
        else:
            await MessageUtils.build_message(
