...
async def get_meta(media_id: int, auth=None, reqtype="both", **kwargs):
    ...
    raw_json = await get(url, cookies=AuthManager.get_cookies(), raw=True, params=params, auth=auth, reqtype=reqtype,
                         **kwargs)
...
async def get_videos(uid: int):
    ...
    async with httpx.AsyncClient(cookies=AuthManager.get_cookies(), headers=headers) as client:
...
async def get_user_dynamics(
    ...
    return await get(url, headers=headers, cookies=AuthManager.get_cookies(), params=params)


async def get_room_info_by_id(
        live_id: int, *, auth=None, reqtype="app", **kwargs
):
    from bilireq.utils import get
    """根据房间号获取指定直播间信息"""
    url = f"https://api.live.bilibili.com/room/v1/Room/get_info"
    params = {"id": live_id}
    return await get(url, cookies=AuthManager.get_cookies(), params=params, auth=auth, reqtype=reqtype, **kwargs)
